...
public class MultiBoxTracker implements TextToSpeech.OnInitListener {
  ...
  public synchronized void draw(final Canvas canvas) {
    canvas.drawRect(220,180,260,240,boxPaint);
    if(trackedObjects.size()>0){logger.i("RESULTS SEEING: ",trackedObjects.get(0).title);}
    ...
    TrackedRecognition closestObject = null;
    ...
    for (final TrackedRecognition recognition : trackedObjects) {
      ...
      if(recognition.detectionConfidence > 0.6 && Math.sqrt(Math.pow(240-(trackedPos.left + trackedPos.right)/2, 2) + Math.pow(240 -(trackedPos.bottom + trackedPos.top)/2,2))<closestObjectDistance){
        closestObjectDistance = (float) Math.sqrt(Math.pow(240-(trackedPos.left + trackedPos.right)/2, 2) + Math.pow(240 -(trackedPos.bottom + trackedPos.top)/2,2));
        closestObject = recognition;
      }
      logger.i("Tracked this, yayyyy!" + recognition.title,recognition.title);
